...
\end{multline}
it's importan to note, that this computation is irrelevant for $ E\sim\Delta $, so $ \Delta $ here acts like hight energy cutoff. 

\subsection{$ E $ is near and below $ \Delta $}

To maker the estimate for the supercurrent at enregies $ E\lesssim \Delta$ high eneregy versions of $ \psi_{low} $ can be used, so the system forgets about $ g_L $ and $ g_R $ and becomes effectively symmetric. Taking the notation from (\ref{s_matrix_2D}), find:
\begin{multline}
	r_{LL}=r_{RR}
	=
	e^{i \gamma }+
