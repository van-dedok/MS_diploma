...
\end{gather}
where everything not important for the estimation is put in the multipliers $ A_N $. 

It's easy to see, that the biggest term in (\ref{ionization_saddle_action}) corresponds to the $ N_*=2M\log\frac{1}{T_\lambda} $. This is the optimal photon number  for the slice for energy range from $ E $ to $ E+\Delta E $ with $ E=\abs{g_L}\lambda  $ and $ \Delta E = \omega M $. Consequentially the optimal photon size is given by $ n_*=\frac{M}{2N_*}=\log\frac{1}{T_\lambda} $.
